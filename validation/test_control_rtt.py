...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Measure C-command round-trip time by matching control seq values against telemetry."
    )
    add_common_arguments(parser)
    parser.add_argument("--duration", type=float, default=60.0, help="Test duration in seconds")
    parser.add_argument("--send-hz", type=float, default=100.0, help="Control send rate")
        ...
        def handle_line(line: str) -> None:
            ...
            with lock:
                if seq in send_times:
                    ...
                    rtts.append(rtt)
                    samples.append({"seq": seq, "rtt_ms": rtt, "foc_rate_hz": fps})
    ...
    if not rtts:
        raise RuntimeError("No RTT data received.")
    ...
    result_path = write_test_result(
        ...
        args.result_dir,
        summary_rows,
        sections=[("rtt_samples", samples)],
    )

    print("Control RTT test passed")
    print(f"Port: {port}")
