...
public:
  ...
  void Sensor_init(TwoWire *_wire);
  void Sensor_update();
  float getAngle();
  float getVelocity();
  float getMechanicalAngle();
  float getSensorAngle();
  uint32_t i2c_error_count = 0;   // Cumulative I2C error count (for telemetry/debug)
...
private:
  ...
  float last_valid_raw_angle = 0; // Last successful I2C read result (fallback on error)
