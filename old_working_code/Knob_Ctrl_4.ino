...
void loop()
{
  ...
  // angle_range 范围外
    if (angle < L2)
      torque = kp_pos1 * (L2 - angle);
    if (angle > L1)
      torque = kp_pos1 * (L1 - angle);

    // 限制最大力矩，防止过载
    const float T_MAX = 1.5; // 最大力矩，根据实际情况调整
    if (torque < -T_MAX)
      torque = -T_MAX;
    if (torque > T_MAX)
      torque = T_MAX;
  ...
  // Print out the current angle and torque for debugging
  Serial.print("Angle: ");
